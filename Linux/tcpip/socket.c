...
int bind(int sockfd, struct sockaddr_in *addr, int addLen)
{
  ...
  {
    return tcpBind(sockets[sockfd].pSockCb, (struct sockaddr_in *)addr,addLen);
  }
  else ERROR;   
...
int socketClose(int sockfd)
{
  ...
  {
      stat = tcpClose(sockets[sockfd].pSockCb,0,0);
  }
  else
   {
     /*********Other Protocol*********/
  ...
  if(((tcpCb_t *)(sockets[sockfd].pSockCb))->state == TIME_WAIT || 
     ((tcpCb_t *)(sockets[sockfd].pSockCb))->state == LAST_ACK)
  {
