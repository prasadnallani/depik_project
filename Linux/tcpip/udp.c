...
* Description :
   This function is called by the socket function . This function 
   allocate memory for one udp control block , initialize the control 
   block ,and adds that control block to udp list.it returns the 
...
* Name: udpBind
* Description :
     This function is called by the bind system call. Responsibility of   the function is fill the port number and ip address in udpcontrol       block ,it adds the state as BIND_BIT set.if all fine then it returns    sucess otherwise it returns the error.
* Parameters :
...
* Description :
   This function is called by the connect system call. Responsibility 
   of this function is fill the port number and ip address of the 
   destination in udp control block, and sets the state as CONNECTED.
   Finally if all are fine it returns SUCCESS otherwise it returns 
   ERROR.
...
* Returns :
      if all are fine then it returns SUCCESS otherwise it returns 
      the ERROR.
***********************************************************************/
...
* Name: udpSendto
* Description :
      This function is called by the sendto() system call.First it sees 
   the whether the state of the control block is CONNECTED or BINDED.if 
   not then it sees whether user sends valid sockaddr_in structure vara-
   ible.if all are fine it allocates memory for one MBuf_t pointer vari-
   able using mbufAlloc() function.This function extracts the source and
   destination port numbers and fills those portnumbers in udp structure
   and also fills the len of the buffer and check sum is zero.Finally we
   send this mbuf to ip layer using the ipSendPkt() function .
              ...
              struct sockaddr_in *pcliAdr, int length) 
{
  ...
  pudp = (udpHdr_t *)(pkt->data - sizeof(udpHdr_t));
  ...
  {
    pudp->dport  = pucb->rport;//this condition will be occured only                                      using by connect system call
...
* Description :
     This function is called by the recvfrom system call. Responsibility
  of this function is recieve the data segment from ip layer. Initially
  it waits on semaphore for the packet . ip layer queues the packet into
  udp control block and gives the semaphore,then this function takes the
  semaphore and dequeues the packet from the control block queue, parse 
  the packet finally it extractes the data from packet and copies into  
  users buffer and also it fills the port number and ip address of the 
  destination machine sockaddr_in structure and fills the size of the   
  structure size in structlen variable.
                ...
                struct sockaddr_in *pAdIn, int *structlen) 
{
  ...
  udpHdrp = (udpHdr_t *)(pkt->data);
  mlen = ntohs(udpHdrp->len) - sizeof(udpHdr_t);
  length = (mlen < len) ? mlen : len;
  memcpy(buf, pkt->data+sizeof(udpHdr_t), length);
  
  if(pAdIn)
  {
    pAdIn->sin_port = udpHdrp->sport;
    memcpy(&pAdIn->sin_addr.s_addr, (pkt->data-IP_HDR_LEN)+12, 4);
  }
  printf("i am in udpRecvFrom\n");
  pktFree(pkt);
  return length;
}
...
* Decription :
       ...
       This function searches the given portnumber in udp control block
  list.If the port number matches in any control block then it returns 
  that pointer other wise it returns NULL.
...
* Description :
      ...
      This function allocates one new port number which is not there in
  udp control block list.it is unique
...
* Name: displayCtlBlk()
* Description :
        This function displays all the UDP control blocks present in the
  linked list.
***********************************************************************/
