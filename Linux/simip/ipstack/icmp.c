...
* Name: icmpIn
* Description:
*   This function is called whenever IP packet with protcol field as ICMP is
*   received. This function processes only ICMP echo request and echo response
*   packets only. If it receives icmp echo request packet, it sends icmp
...
void icmpIn(pktBuf_t *pkt)
{
  ...
  pic = (icmpHdr_t *) &pkt->buf[ETH_HDR_LEN + IP_HDR_LEN];
  switch(pic->icmpType)
  {
    case ICMP_ECHO_REQ:
