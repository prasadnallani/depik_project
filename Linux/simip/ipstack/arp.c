...
void arpIn(pktBuf_t *pkt)
{
  ...
  if(op == ARP_REQ)
  {
    ...
    if(memcmp(pArp->targetPra,pIf->ipAdrs,4) == 0)
    {
      ...
      memcpy(&pkt->buf[0], &pkt->buf[6], HW_ADRS_LEN);
      memcpy(&pkt->buf[6], pIf->macAdrs, HW_ADRS_LEN);
  ...
  else if(op == ARP_RESP)
  {
    ...
    for(ii=0; ii<MAX_ARP_ENTRIES; ii++)
    {
           ...
           (memcmp(arpTbl[ii].ipAdrs,pArp->senderPra,4) == 0))
      {
        ...
        {
          memcpy(pkt->buf,arpTbl[ii].hwAdrs,HW_ADRS_LEN);
          arpSentIpPktWRCnt++;
          pIf->sendFP(pIf->pDev,pkt);
        }
        return;
...
* Name: arpSendIpPkt
* Description:
*   This function is called by the IP layer to send an IP packet. This arp
*   function should fill the destination's hardware address in packet's
*   frame header.
*******************************************************************************/
int arpSendIpPkt(pktBuf_t *pkt, UINT8 *nxtHopIpAdrs, iface_t *pIf)
{
  ...
  {
    arpHitCnt++;
    memcpy(pkt->buf,arpTbl[index].hwAdrs,HW_ADRS_LEN);
    //printf("ARP SEND pkt len %d\n",pkt->len);
    pIf->sendFP(pIf->pDev,pkt);
  }
  else
  {
    ...
    pkt->len = sizeof(arpPkt_t) + ETH_HDR_LEN;
    arpReqSentCnt++;
