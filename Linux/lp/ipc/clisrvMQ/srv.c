...
{
  int msgQId, msgLen;
  char pCmdMsg[MSG_LEN];
  char pRespMsg[MSG_LEN];
  
  msgQId = msgget(MY_KEY, 0600 | IPC_CREAT);
  if(msgQId < 0)
  {
    printf("Could not create msgQ\n");
    return(1);
  }
  else
  ...
  { 
    ...
    msgLen = msgrcv(msgQId, pCmdMsg, MSG_LEN, SRV_MSG_TYPE, 0);
    if(msgLen == -1)
    {
      printf("Error: In receiving msg from MsgQ\n");
      return(1);
    }
    printf("Recieved message of size %d bytes\n", msgLen);
    processDbCmdmsg(pCmdMsg + 8, pRespMsg + 4);
    memcpy(pRespMsg, pCmdMsg + 4, 4);
   
    msgsnd(msgQId, pRespMsg, MSG_LEN, SRV_MSG_TYPE, 0);
