...
int TxAndRxDbCmdmsgThroughMsgQ(Uchar *pCmdMsg, Uchar *pRespMsg);
...
main(void)
{
  ...
  while(1)
  {
    ...
    switch(userReq)
    {
      case ADD:
		getEmpDetails4mClient(rec);
                frameMessage(ADD, (void*)rec, pCmdMsg+8);
	        TxAndRxDbCmdmsgThroughMsgQ(pCmdMsg, pRespMsg);
	...
	case MOD:
		getEmpDetails4mClient(rec);
                frameMessage(MOD, (void*)rec, pCmdMsg+8);
	        TxAndRxDbCmdmsgThroughMsgQ(pCmdMsg, pRespMsg);
                if(pRespMsg[0] == SUCCESS)
	...
	case DEL:
		...
		scanf("%d", &empId);
                frameMessage(DEL, (void*)&empId, pCmdMsg+8);
	        TxAndRxDbCmdmsgThroughMsgQ(pCmdMsg, pRespMsg);
                if(pRespMsg[0] == SUCCESS)
	...
	case GET:
		...
		scanf("%d", &empId);
                frameMessage(GET, (void*)&empId, pCmdMsg+8);
	        TxAndRxDbCmdmsgThroughMsgQ(pCmdMsg, pRespMsg);
                if(pRespMsg[0] == SUCCESS)
                {
...
int TxAndRxDbCmdmsgThroughMsgQ(Uchar *pCmdMsg, Uchar *pRespMsg)
{
  int msgQId, msgLen;
  int cliPID;
  int *intPHdr = (int*)pCmdMsg;
  Uchar tempBuff[MSG_LEN];
  
   msgQId = msgget(MY_KEY, 0600);
   if(msgQId < 0)
   {
     printf("Client:Could not open msgQ\n");
     return(1);
   }
   intPHdr[0] = SRV_MSG_TYPE;
   cliPID = getpid();
   intPHdr[1] = cliPID;

   msgsnd(msgQId, pCmdMsg, MSG_LEN, SRV_MSG_TYPE, 0);
   printf("Client:Sent given msg to server\n");
   msgLen = msgrcv(msgQId, tempBuff, MSG_LEN, cliPID, 0);
   memcpy(pRespMsg, tempBuff + 4, msgLen);
}
