...
static unsigned char s3c4510_eth_addr[ETH_ALEN] = 
{
...
{
  //struct eth_cfg eth; //karasala
  int i;

  //bios_eth_cfg(&eth); //karasala
  //memcpy(s3c4510_eth_addr, eth.address, ETH_ALEN); //karasala
...
int s3c4510_eth_send(unsigned char *data, int len)
{
  ...
  fd_ptr->reserved = (Padding | CRCMode | FrameDataPtrInc | LittleEndian | 
                      WA00 | MACTxIntEn);
